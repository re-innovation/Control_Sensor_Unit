...
void loop(){
  ...
  while(weightData<-1.00)
  {
    // There is a weight, so turn on the power and play the tune
    digitalWrite(relay,HIGH);
    digitalWrite(colourLED, HIGH); // Switch ON LEDs as indicator 
    if(outputData==HIGH)
    {
      Serial.println("PLAY?");  
      outputData = LOW;
  ...
  digitalWrite(colourLED, LOW); // Switch OFF LEDs as indicator 
